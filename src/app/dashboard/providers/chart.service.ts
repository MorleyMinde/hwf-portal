...
export class ChartService {
      ...
      if(chartDataFromStore != null) {
        ...
        if(chartData.details.hasOwnProperty('favorite')) {
          ...
          if(favoriteType != null && favoriteId != null) {
            ...
            alert("Here:" + this.constant.api + favoriteType)
  ...
  public getChartObjects(chartData: Visualization, chartType: string = null): any[] {
    ...
    if(chartData.layers.length > 0) {
      chartData.layers.forEach((layer, index) => {
        const renderId: string = chartData.id + '_' + index;
        /**
        ...
        if(layer.hasOwnProperty('analytics') && layer.analytics != undefined) {
          if(layer.analytics.hasOwnProperty('headers')) {
            let chartObject = this.visualizationService.drawChart(layer.analytics, layer.settings.chartConfiguration);
